...
export default {
  ...
  methods: {
    async updateNotesList() {
      const node = this.nodeInfo.selectedNode;
      this.notesDisplayList = await noteController.getAndFormatNotes(node);
    ...
    resetBind() {
      if (this.nodeInfo && this.nodeInfo.selectedNode) {
        if (typeof this.myBind !== "undefined") {
